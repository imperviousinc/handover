...
class Plugin {
  constructor(node) {
    ...
    this.ns.middle = async (tld, req) => {
      ...
      // Check NS records for HIP-005 referrals
      for (const rr of res.authority) {
        ...
        // Look for any supported HIP-005 extension in the NS record
      ...
      // If the HIP-005 stuff came up empty, return the
      // HNS root server response unmodified.
      if (!data || data.length === 0) {
        // Never send HIP-005 type referrals to recursive resolvers
